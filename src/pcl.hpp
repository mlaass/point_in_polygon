...
struct tensor_features {
  ...
  template <typename vec3> std::vector<double> operator()(const vec3 &l) {
    return {
...
public:
  ...
  bool loadh5(std::string filename, bool index = true) {
    ...
    auto filespace = H5Dget_space(coords_ds); /* Get filespace handle first. */
  ...
  bool load(std::string filename) {
    ...
    while (std::getline(ifs, line)) {
      ...
      if (tokens.size() != 5)
        throw(std::runtime_error("need five tokens on each non-comment line"));
